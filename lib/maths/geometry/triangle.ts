...
export class Triangle {
    ...
    parse = (...values: any): Triangle => {
        if (values.length === 6) {
            ...
            const v = values.map((x: any) => new Fraction(x))
            return this.parse(
                new Vector(v[0], v[1]),
                new Vector(v[2], v[3]),
                new Vector(v[4], v[5]),
            ...
            if (values.filter((x: any) => typeof x === 'string').length === 3) {
                ...
                if (values.filter((x: any) => (x instanceof Vector)).length < 3) {
                    return this.parse(
                        new Vector(values[0]),
                        new Vector(values[1]),
                        new Vector(values[2])
     ...
     * @param ptName
     */
    private getPointByName = (ptName: string): Vector => {
    ...
    private _calculateBisectors = (pt: string): { internal: Line, external: Line } => {
        ...
        const b1 = new Line(new Equation(d1.equation.left.clone().multiply(d2.n.simplify().norm), d2.equation.left.clone().multiply(d1.n.simplify().norm)).reorder(true).simplify()),
